...
const ChatWidgetStyles = () => (
    <style>{`
        /* --- ADD THIS NEW STYLE FOR OUR WELCOME MESSAGE --- */
        .chat-resume-message {
            text-align: center;
            padding: 1rem 0;
            font-size: 0.8rem;
        ...
        .chat-message .msg-timestamp { font-size: 0.7rem; margin-top: 4px; opacity: 0.7; display: block; text-align: right; }
        ...
        .typing-indicator-container { height: 20px; padding: 0 1rem; box-sizing: border-box; }
        .typing-indicator { font-style: italic; font-size: 0.8rem; color: var(--text-secondary); animation: fadeIn 0.3s ease-in-out; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .read-receipt { margin-left: 5px; color: var(--text-secondary); font-size: 0.8rem; }
        .read-receipt.read { color: var(--accent-blue); font-weight: bold; }
    `}</style>
);
...
const ChatWidget = () => {
    ...
    useEffect(() => {
        ...
        // We check the length of the initial history provided by the server.
        if (customerChat.history && customerChat.history.length > 0) {
            setIsResumedSession(true);
        }
    }, [customerChat.history]); // This effect depends on the initial history array
