...
const prestigeUpgrades = [
    { id: 'permanentDamage', name: 'Exalted Power', description: 'Permanently increase all damage by 10% per level.', cost: 1, type: 'damage_multiplier' },
    { id: 'permanentFame', name: 'Exalted Wealth', description: 'Start each new run with 1000 Fame per level.', cost: 2, type: 'starting_score' },
    { id: 'permanentPPS', name: 'Exalted Will', description: 'Permanently gain 50 DPS per level.', cost: 3, type: 'starting_pps' }
];
...
function formatTime(totalSeconds) {
    const days = Math.floor(totalSeconds / 86400); totalSeconds %= 86400; const hours = Math.floor(totalSeconds / 3600); totalSeconds %= 3600; const minutes = Math.floor(totalSeconds / 60); const seconds = totalSeconds % 60;
    let parts = [];
    if (days > 0) parts.push(`${days} day${days > 1 ? 's' : ''}`); if (hours > 0) parts.push(`${hours} hour${hours > 1 ? 's' : ''}`); if (minutes > 0) parts.push(`${minutes} minute${minutes > 1 ? 's' : ''}`); if (seconds > 0 || parts.length === 0) parts.push(`${seconds} second${seconds !== 1 ? 's' : ''}`);
    return parts.join(', ');
}
...
function PixelClickerGame() {
    ...
    const playerAbility = abilities.find(a => a.classId === gameState.playerClass);

    const playSound = (soundFile, volume = 1) => { if (!gameState.isMuted) { try { const audio = new Audio(soundFile); audio.volume = volume; audio.play().catch(e => console.error("Audio play failed:", e)); } catch (e) { console.error("Audio creation failed:", e); } } };
    const calculateAchievementBonuses = () => { const bonuses = { clickDamageMultiplier: 1, fameMultiplier: 1, clickDamageFlat: 0, shardMultiplier: 1, }; for (const ach of achievements) { if (gameState.unlockedAchievements[ach.id]) { switch (ach.reward.type) { case 'CLICK_DAMAGE_MULTIPLIER': bonuses.clickDamageMultiplier += ach.reward.value; break; case 'FAME_MULTIPLIER': bonuses.fameMultiplier += ach.reward.value; break; case 'CLICK_DAMAGE_FLAT': bonuses.clickDamageFlat += ach.reward.value; break; case 'SHARD_MULTIPLIER': bonuses.shardMultiplier += ach.reward.value; break; default: break; } } } return bonuses; };
    ...
    useEffect(() => {
        const timerInterval = setInterval(() => {
            ...
            if (playerAbility) {
                ...
                updatedTimers[playerAbility.id] = Math.max(0, Math.ceil((cdEnd - now) / 1000));
            }
    ...
    useEffect(() => {
        ...
        if (activeBuffs[buffId]) {
            const timer = setTimeout(() => {
                setActiveBuffs(prev => { const newBuffs = { ...prev }; delete newBuffs[buffId]; return newBuffs; });
    ...
    useEffect(() => { for (const ach of achievements) { if (!gameState.unlockedAchievements[ach.id] && ach.isUnlocked(gameState)) { setGameState(prev => ({ ...prev, unlockedAchievements: { ...prev.unlockedAchievements, [ach.id]: true } })); toast.custom((t) => ( <div className={`achievement-alert ${t.visible ? 'animate-enter' : 'animate-leave'}`} onClick={() => toast(ach.rewardDescription, { icon: '🏆' })}> <strong>🏆 Achievement Unlocked!</strong> <p>{ach.name}</p> <small>Click to see reward!</small> </div> )); } } }, [gameState.totalClicks, gameState.totalFameEarned, gameState.bossesDefeated, gameState.hasPrestiged]);
    useEffect(() => { if (gameState.equippedWeapon !== 'stacking_vipers') { if (poison.stacks > 0) setPoison({ stacks: 0, lastApplied: null }); return; } const poisonInterval = setInterval(() => { if (poison.stacks > 0) { if (Date.now() - poison.lastApplied > 3000) { setPoison(p => ({ ...p, stacks: Math.max(0, p.stacks - 1) })); } const poisonDps = poison.stacks * (1 + Math.floor(gameState.currentBossIndex * 1.5)); setGameState(prev => ({ ...prev, clicksOnCurrentBoss: prev.clicksOnCurrentBoss + poisonDps })); } }, 1000); return () => clearInterval(poisonInterval); }, [gameState.equippedWeapon, poison]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => { setGameState(prev => ({ ...prev, score: prev.score + fameFromDps, totalFameEarned: prev.totalFameEarned + fameFromDps })); }, 1000);
        return () => clearInterval(interval);
    }, [gameState.pointsPerSecond, gamePhase, isHealing, gameState.unlockedAchievements, gameState.equippedWeapon, activeBuffs]);
    
    useEffect(() => { if (!currentBoss || gamePhase !== GAME_PHASES.CLICKING || isHealing) return; if (gameState.clicksOnCurrentBoss >= currentBoss.clickThreshold) { setGameState(prev => ({ ...prev, bossesDefeated: { ...prev.bossesDefeated, [currentBoss.id]: (prev.bossesDefeated[currentBoss.id] || 0) + 1 } })); if (currentBoss.id === 'oryx3') { setGamePhase(GAME_PHASES.EXALTED_TRANSITION); return; } playSound(currentBoss.breakSound); if (gameState.currentBossIndex === bosses.length - 1) { setGameWon(true); setGamePhase(GAME_PHASES.FINISHED); } else { setGamePhase(GAME_PHASES.TRANSITIONING); } } }, [gameState.clicksOnCurrentBoss, gameState.currentBossIndex, currentBoss, gamePhase, isHealing]);
    ...
    const calculateDamageRange = () => {
        ...
        switch (gameState.equippedWeapon) { case 'executioners_axe': minDamage *= 0.75; maxDamage *= 0.75; break; case 'golden_rapier': minDamage *= 0.80; maxDamage *= 0.80; break; case 'stacking_vipers': minDamage *= 0.20; maxDamage *= 0.20; break; default: break; }
        minDamage += bonuses.clickDamageFlat; maxDamage += bonuses.clickDamageFlat;
        ...
        minDamage = minDamage * damageMultiplier * bonuses.clickDamageMultiplier; maxDamage = maxDamage * damageMultiplier * bonuses.clickDamageMultiplier;
        if(activeBuffs['arcane_power']) { minDamage *= 2; maxDamage *= 2;}
    ...
    const handleUseAbility = (abilityId) => {
        ...
        switch (abilityId) {
            case 'slam': {
                ...
                setGameState(prev => ({ ...prev, score: prev.score + fameFromSlam, clicksOnCurrentBoss: prev.clicksOnCurrentBoss + slamDamage }));
                setFloatingNumbers(current => [...current, { id: uuidv4(), value: fameFromSlam, x: window.innerWidth / 2, y: window.innerHeight / 2, className: 'slam-damage'}]);
                toast.success('SLAM!', { icon: '💥' });
                break;
            }
            case 'arcane_power': {
                setActiveBuffs(prev => ({ ...prev, [ability.id]: { expiresAt: now + 10000 } }));
            ...
            case 'virulent_outbreak': {
                if (gameState.equippedWeapon === 'stacking_vipers') {
                    ...
                    setGameState(prev => ({ ...prev, clicksOnCurrentBoss: prev.clicksOnCurrentBoss + flatDamage }));
        ...
        setGameState(prev => ({ ...prev, abilityCooldowns: { ...prev.abilityCooldowns, [abilityId]: now + ability.cooldown * 1000 } }));
    };
    ...
    const handleGemClick = (event) => {
        ...
        const bonuses = calculateAchievementBonuses();
        switch (gameState.equippedWeapon) { case 'executioners_axe': if (Math.random() < 0.10) { damageDealt *= 10; toast('CRITICAL HIT!', { icon: '💥', duration: 1000 }); } break; case 'golden_rapier': fameEarned *= 1.25; break; case 'stacking_vipers': setPoison(p => ({ stacks: p.stacks + 1, lastApplied: Date.now() })); break; default: break; }
        
        const isArcaneFameProc = activeBuffs['arcane_power'] && Math.random() < 0.25;

        if (isArcaneFameProc) {
            let dps = gameState.pointsPerSecond * 2; // Arcane power doubles dps
             const fameFromAbility = Math.floor(dps * bonuses.fameMultiplier);
             setGameState(prev => ({ ...prev, score: prev.score + fameFromAbility }));
            ...
            setGameState(prev => ({ ...prev, score: prev.score + fameEarned, clicksOnCurrentBoss: prev.clicksOnCurrentBoss + damageDealt, totalClicks: prev.totalClicks + 1, totalFameEarned: prev.totalFameEarned + fameEarned, }));
        }
        setIsShaking(true);
        setTimeout(() => setIsShaking(false), 150);
    };

    const handleEnterPortal = () => { setGameState(prev => ({ ...prev, currentBossIndex: prev.currentBossIndex + 1, clicksOnCurrentBoss: 0, temporaryUpgradesOwned: {}, abilityCooldowns: {} })); setGamePhase(GAME_PHASES.CLICKING); };
    ...
    const handlePrestige = () => {
        ...
        if (shardsToAward < 1) { alert("You need a higher score to prestige! Try reaching at least 2,500,000 Fame."); return; }
        ...
        if (isConfirmed) {
            setGameState(prev => {
                ...
                return { ...defaultState, playerClass: prev.playerClass, exaltedShards: prev.exaltedShards + shardsToAward, prestigeUpgradesOwned: prev.prestigeUpgradesOwned, score: startingScore, pointsPerSecond: startingPps, totalClicks: prev.totalClicks, totalFameEarned: prev.totalFameEarned, unlockedAchievements: prev.unlockedAchievements, bossesDefeated: {}, hasPrestiged: true, isMuted: prev.isMuted, unlockedWeapons: prev.unlockedWeapons };
            });
            setGameWon(false);
    ...
    const handleUnlockWeapon = (weapon) => { if (gameState.exaltedShards >= weapon.cost) { setGameState(prev => ({ ...prev, exaltedShards: prev.exaltedShards - weapon.cost, unlockedWeapons: { ...prev.unlockedWeapons, [weapon.id]: true } })); toast.success(`Unlocked ${weapon.name}!`); } else { alert("Not enough Exalted Shards!"); } };
    const handleEquipWeapon = (weaponId) => { setGameState(prev => ({ ...prev, equippedWeapon: weaponId })); toast.success(`Equipped ${weapons.find(w => w.id === weaponId)?.name || 'Default Sword'}!`); };
    ...
    return (
        <>
            ...
            <div className="game-hud"> <button onClick={toggleMute} className="btn-mute"> {gameState.isMuted ? '🔇' : '🔊'} </button> </div>
            ...
            <div className="card">
                ...
                {floatingHeals.map(num => ( <span key={num.id} className="floating-number heal" style={{ left: num.x, top: num.y }} onAnimationEnd={() => setFloatingHeals(current => current.filter(n => n.id !== num.id))}> +{num.value.toLocaleString()} </span> ))}
                <div className="clicker-container">
                    ...
                    {gamePhase === GAME_PHASES.CLICKING && ( <p> {gameState.pointsPerSecond.toLocaleString()} DPS / {calculateDamageRange().minDamage.toLocaleString()}-{calculateDamageRange().maxDamage.toLocaleString()} per click </p> )}
                    ...
                    <div className="ability-bar">
                        {playerAbility && (
                            <button className="ability-button" onClick={() => handleUseAbility(playerAbility.id)} disabled={cooldownTimers[playerAbility.id] > 0}>
                                <div className="ability-info">
                                    <strong>{playerAbility.name}</strong>
                                    <small>{playerAbility.description}</small>
                                </div>
                                {cooldownTimers[playerAbility.id] > 0 && <div className="cooldown-overlay"><div className="cooldown-text">{cooldownTimers[playerAbility.id]}s</div></div>}
                            </button>
                        )}
                    </div>
                    ...
                    {(gamePhase === GAME_PHASES.CLICKING || isInvulnerable) && (
                        <>
                            ...
                            {activeShop === 'prestige' && ( <div className="upgrades-shop"> <h4 style={{ color: '#8a2be2' }}>Prestige Shop</h4> <div className="upgrades-grid"> {prestigeUpgrades.map(up => { const cost = calculatePrestigeUpgradeCost(up); return ( <button key={up.id} onClick={() => handleBuyPrestigeUpgrade(up)} className="btn-upgrade prestige" disabled={gameState.exaltedShards < cost || isHealing}> <span className="upgrade-name">{up.name}</span> <small>{up.description}</small> <small>Cost: {cost} Shards</small> <small>(Level: {gameState.prestigeUpgradesOwned[up.id] || 0})</small> </button> ); })} {weapons.map(w => !gameState.unlockedWeapons[w.id] && (<button key={w.id} onClick={() => handleUnlockWeapon(w)} className="btn-upgrade weapon-unlock" disabled={gameState.exaltedShards < w.cost || isHealing}><span className="upgrade-name">Unlock: {w.name}</span><small>{w.description}</small><small>Cost: {w.cost} Shards</small></button>))} </div> </div> )}
                            {activeShop === 'achievements' && ( <div className="upgrades-shop"> <h4>Achievements</h4> <div className="achievements-grid"> {achievements.map(ach => ( <div key={ach.id} className={`achievement-item ${gameState.unlockedAchievements[ach.id] ? 'unlocked' : ''}`}> <strong className="achievement-name">{ach.name}</strong> <p className="achievement-desc">{ach.description}</p> <p className="achievement-reward">{gameState.unlockedAchievements[ach.id] ? ach.rewardDescription : '???'}</p> </div> ))} </div> </div> )}
                            {activeShop === 'armory' && ( <div className="upgrades-shop"> <h4>Armory</h4> <div className="armory-grid"> <div key="default" className={`weapon-item ${gameState.equippedWeapon === 'default' ? 'equipped' : ''}`} onClick={() => handleEquipWeapon('default')}> <strong>Default Sword</strong> <p>Your trusty, reliable blade. No special effects.</p> </div> {weapons.map(w => ( gameState.unlockedWeapons[w.id] && ( <div key={w.id} className={`weapon-item ${gameState.equippedWeapon === w.id ? 'equipped' : ''}`} onClick={() => handleEquipWeapon(w.id)}> <strong>{w.name}</strong> <p>{w.description}</p> <div className="weapon-pro-con"> {w.pro_con.map((eff, index) => ( <small key={index} className={eff.type}>{eff.text}</small> ))} </div> </div> ) ))} </div> </div> )}
                        </>
                    )}
                </div>
            </div>
